...
<?php
  ...
  if($user_info = mysqli_query($conn,"SELECT * FROM delivery WHERE username='$userid'"))
{
...
?>
		...
		<tr>
        	<td scope="row" data-label="Order ID"><?php echo $row2['id'] ?></td>
			<td data-label="Order Location"><?php echo $row2['orderlocation'] ?></td>
			<td data-label="Pickup Name"><?php echo $row2['ordername'] ?></td>
      <td data-label="Pickup Time"><?php echo $row2['pickuptime'] ?></td>
			<td data-label="Delivery Address"><?php echo $row2['deliveryaddress'] ?></td>
			<td data-label="Phone Number"><?php echo $row2['phonenumber'] ?></td>
			<td data-label="Delivery Status"><?php echo $row2['status'] ?></td>
			<td data-label="Pay"><?php
            if($row2['paytype'] == 'Card' && $row2['status'] == $paymentcheck){
			echo "Order Marked As Paid";
			}
             if($row2['paytype'] == 'Card' && $row2['status'] != $paymentcheck){
             echo "<form action='pay.php' method='post'><button type='submit' name='payorderid' value='". $row2['id'] ."' style='width:100%'>Pay</button></form>";
            }else if ($row2['paytype'] == 'Cash'){
            echo "Paying Cash";
            }?></td>

            <td data-label="Cancel Delivery"><?php echo "<form action='canceldelivery.php' method='post'><button type='submit' name='payorderid' value='". $row2['id'] ."' style='width:100%'>Cancel</button></form>";?></td>
		</tr>
<?php
}}}
?>
