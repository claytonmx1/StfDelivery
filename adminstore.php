...
<html>
	...
	<body class="subpage">
		...
		<?php
		$openresults = mysqli_query($conn,"SELECT * FROM areweopen");
		$openorclosedfetch = mysqli_fetch_assoc($openresults);
		$areweopen = $openorclosedfetch['store'];
		?>
			<section id="main" class="wrapper">
				<div class="inner">
					<header class="align-center">
						<h1>Admin Open Or Closed Setting</h1>
						<h1>Store Status - <?php echo $areweopen; ?></h1>
						<p></p>
					</header>
					<!--<div class="image fit">
						<img src="images/saintfranhouse.jpg" alt="" />
                    </div>  no image for our delivery form-->
									<form name="form1" id="form1" method="post" action="openorclosed.php">
										<div class="row uniform">
                                            <div class="6u 12u$(xsmall)">
											<label for="name">Open / Close</label>
												<input type="text" name="openorclosed" id="openorclosed" value="" placeholder="Open Or Closed" />
											</div>
											<!-- Break -->
											<!-- Break -->
											<!-- Break -->
											<div class="12u$">
												<ul class="actions">
													<li><button type="submit" name="submit" onclick="return checkform()">Open / Shutdown</button></li>
												</ul>
											</div>
										</div>
									</form>
